...
async fn refresh_antigravity_token(
    ...
    let client_id = env!(
        ...
        "Set ANTIGRAVITY_CLIENT_ID env var (see .env.example)"
    );
    let client_secret = env!(
        "ANTIGRAVITY_CLIENT_SECRET",
        "Set ANTIGRAVITY_CLIENT_SECRET env var (see .env.example)"
    );
...
{
    use crate::types::{AntigravityModelsResponse, AntigravityQuotaResult, ModelQuota};
                        ...
                        match refresh_antigravity_token(&client, rt).await {
                            Ok(new_token) => {
                                ...
                                if let Some(obj) = auth_json.as_object_mut() {
                                    ...
                                    let new_expiry = (chrono::Local::now()
                                        + chrono::Duration::hours(1))
                                    .to_rfc3339();
                                    obj.insert(
                                        "expired".to_string(),
                                        serde_json::Value::String(new_expiry),
                                    );
                ...
                'url_loop: for base_url in base_urls {
                    ...
                    match response {
                        Ok(resp) => {
                            if resp.status() == 403 {
                                last_error = Some(
                                    "Account forbidden (possibly banned or token expired)"
                                        .to_string(),
                                );
                            ...
                            if resp.status() == 404 {
                                last_error = Some(format!(
                                    "API error: {} (trying next endpoint)",
                                    resp.status()
                                ));
    ...
    if copilot_api_token_path.exists() {
        match std::fs::read_to_string(&copilot_api_token_path) {
            Ok(t) => {
                ...
                if !token.is_empty() {
                    credentials.push(CopilotCred {
                        token,
                        login: "copilot-api".to_string(),
                    });
                }
            }
            Err(e) => {
...
async fn fetch_copilot_quota_with_token(
    token: &str,
    login: &str,
    ...
    match response {
        Ok(resp) => {
            if resp.status().is_success() {
                ...
                let plan = body["copilot_plan"]
                    .as_str()
                    .or_else(|| body["copilotPlan"].as_str())
                    .unwrap_or("unknown")
                    .to_string();

                let quota_snapshots = body
                    .get("quota_snapshots")
                ...
                let (premium_percent, chat_percent) = if let Some(snapshots) = quota_snapshots {
                    ...
                    let premium_pct = premium
                        .and_then(|p| {
                            p.get("percent_remaining")
                                .or_else(|| p.get("percentRemaining"))
                        })
                        .and_then(|v| v.as_f64())
                        .unwrap_or(100.0);
                    ...
                    let chat_pct = chat
                        .and_then(|c| {
                            c.get("percent_remaining")
                                .or_else(|| c.get("percentRemaining"))
                        })
                        .and_then(|v| v.as_f64())
                        .unwrap_or(100.0);
        ...
        Err(e) => crate::types::CopilotQuotaResult {
            account_login: login.to_string(),
            plan: "unknown".to_string(),
            premium_interactions_percent: 0.0,
            chat_percent: 0.0,
...
pub async fn fetch_claude_quota() -> Result<Vec<crate::types::quota::ClaudeQuotaResult>, String> {
    ...
    if claude_creds_path.exists() {
        ...
        let access_token = match cred["accessToken"]
            .as_str()
